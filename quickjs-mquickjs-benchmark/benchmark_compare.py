#!/usr/bin/env python3
"""
Benchmark script for comparing mquickjs sandbox implementations and the quickjs python package.

Compares:
1. FFI implementation (ctypes)
2. Subprocess implementation
3. C Extension (if available)
4. Wasmtime implementation
5. quickjs python package

Run with: python3 benchmark_compare.py
"""
...
# Add mquickjs-sandbox to sys.path
sys.path.append(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "mquickjs-sandbox"))
...
def main():
    print("QuickJS vs mquickjs Benchmark Comparison")
    print("="*60)

    all_results = {}

    # Test quickjs python package
    print("\nTesting quickjs (python package)...")
    try:
        import quickjs

        # Helper class to match mquickjs interface
        class QuickJSWrapper:
            ...
            def close(self):
                # quickjs context is garbage collected
                pass

        # Startup time
        startup = measure_startup_time(QuickJSWrapper)
        print(f"  Startup time: {startup['mean_ms']:.3f}ms")

        # Execution benchmarks
        sandbox = QuickJSWrapper()
        results = run_benchmark(sandbox.execute, "quickjs")
        sandbox.close()

        results["_startup"] = startup
        all_results["quickjs"] = results
        print_results(results, "quickjs Implementation")

    except ImportError as e:
        print(f"  quickjs not available: {e}")
    except Exception as e:
